...
nyp_url = 'https://nypost.com/search/donald+trump/'
source = requests.get(nyp_url).text
soup = BeautifulSoup(source, 'html.parser')

base_soup = soup.find_all('h3', class_='story__headline headline headline--archive')
base_list = []

for ita in base_soup:
    sta = ita.find('a')
    base_list.append(sta)

nyp_url2 = 'https://nypost.com/search/donald+trump/page/2/'
source2 = requests.get(nyp_url2).text
soup2 = BeautifulSoup(source2, 'html.parser')

base_soup2 = soup2.find_all('h3', class_='story__headline headline headline--archive')

for ita2 in base_soup2:
    sta2 = ita2.find('a')
    base_list.append(sta2)
...
def work(num_ind):
    ...
    base_text_soup = mini_soup.find('div', class_='single__content entry-content m-bottom')
...
df.to_csv('Trump_NYP_Scrape_Mod1_Pos_Neg.csv', mode='w')
