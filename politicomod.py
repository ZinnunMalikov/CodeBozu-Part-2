...
def sentiment_analyze(obj, num):
    ...
    if num == 1:
        ...
        pos_df.to_csv('Mod_Trump_Politico_Scrape_Positive_Percent.csv', mode='w')
    ...
    if num == 0:
        ...
        neu_df.to_csv('Mod_Trump_Politico_Scrape_Neutral_Percent.csv', mode='w')
    ...
    if num == -1:
        ...
        neg_df.to_csv('Mod_Trump_Politico_Scrape_Negative_Percent.csv', mode='w')
        
    #print("Overall sentiment dictionary is : ", sentiment_dict)
    if num == -2:
        mult_array = copy.deepcopy(obj)
        for rw in range(np.shape(obj)[0]-1):
            for cl in range(np.shape(obj)[1]):
                sentiment_dict = sid_obj.polarity_scores(obj[rw+1][cl])
                mult_score = round((sentiment_dict['pos']-sentiment_dict['neg'])*100, 3)
            
                mult_array[rw+1][cl] = mult_score

        move_mult=0
        for rw in range(np.shape(mult_array)[0]-1):
            move_mult = move_mult + float(mult_array[rw+1][0])/(np.shape(mult_array)[0]-1)

        col_mult=0
        for rw in range(np.shape(mult_array)[0]-1):
            col_mult = col_mult + float(mult_array[rw+1][1])/(np.shape(mult_array)[0]-2)

        ups_mult=0
        for rw in range(np.shape(mult_array)[0]-1):
            ups_mult = ups_mult + float(mult_array[rw+1][2])/(np.shape(mult_array)[0]-2)

        avgs_mult = np.array([['Average', round(move_mult, 3), round(col_mult, 3), round(ups_mult, 3)]])

        mult_array = np.append(things_array, mult_array, axis=1)

        mult_array = np.append(mult_array, avgs_mult, axis=0)

        mult_array = np.delete(mult_array, 0, 0)

        mult_df = pd.DataFrame(mult_array, columns = ['Things', 'Move (Positive - Negative %)', 'Impact (Positive - Negative %)', 'Upshot (Positive - Negative %)'])

        print(mult_df)

        mult_df.to_csv('Mod_Trump_Politico_Scrape_Pos_Neg_Percent.csv', mode='w')
        
sentiment_analyze(semifinal_array, 1)
sentiment_analyze(semifinal_array, 0)
sentiment_analyze(semifinal_array, -1)
sentiment_analyze(semifinal_array, -2)
